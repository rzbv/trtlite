...
bool IContext::saveEngine(const std::string& engine_file) {
  ...
  // trt10: 序列化网络方式，跳过icuda_engine的创建
  TrtUniquePtr<IHostMemory> serializedEngine{getIBuilder()->buildSerializedNetwork(*getNetWorkDefine(), *getIBuilderConfig())};
...
bool IContext::loadEngine(const std::string& engine_file) {  
  ...
  for (int32_t i = 0; i < _cuda_engine->getNbIOTensors(); ++i) {
    const char* tensor_name = _cuda_engine->getIOTensorName(i);
    if (_cuda_engine->getTensorIOMode(tensor_name) == nvinfer1::TensorIOMode::kINPUT) {
      _inputNames.push_back(tensor_name);
    } else {
      _outputNames.push_back(tensor_name);
    }
  }
