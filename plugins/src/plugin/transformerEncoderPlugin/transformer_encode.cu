...
__global__ void
transpose(
    T* src, T* dst, const int batch_size, const int seq_len, const int head_num,
...
__global__ void
softmax_kernel(
    T* qk_buf_, const T* attr_mask, const int batch_size, const int head_num,
    const int seq_len, const T scalar)
{
  ...
  for (int i = 0; i < seq_len; ++i) {
    ...
    float tmp =
        threadIdx.x < seq_len ? (float)(qk * (float)scalar + mask_val) : NN;
...
__global__ void
add_QKV_bias(
    T* Q, T* K, T* V, T* q_buf_, T* k_buf_, T* v_buf_, const int batch_size,
...
multiHeadAttr_nofuse_kernelLauncher(
    cudaStream_t stream, cublasHandle_t cublas_handle, T* Q, T* K, T* V,
    T* q_buf_, T* k_buf_, T* v_buf_, T* qk_buf_, T* transpose_dst_,
    const T* attr_mask, T* dst, const int batch_size, const int seq_len,
    const int head_num, const int size_per_head, const T scalar)
{
  int m = batch_size * seq_len;
  ...
  if (1) {
    ...
    dim3 block(k);
    add_QKV_bias<T><<<grid, block, 0, stream>>>(
        Q, K, V, q_buf_, k_buf_, v_buf_, batch_size, seq_len, head_num,
  ...
  check_cuda_error(cublasGemmStridedBatchedEx(
      cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N, seq_len, seq_len, size_per_head,
      &alpha, k_buf_, AType_, size_per_head, seq_len * size_per_head, q_buf_,
      BType_, size_per_head, seq_len * size_per_head, &beta, qk_buf_, CType_,
      seq_len, seq_len * seq_len, batch_size * head_num, computeType_,
  ...
  if (batch_size * head_num <= 120) {
    ...
    softmax_kernel_v2<T><<<grid, block, 0, stream>>>(
        qk_buf_, attr_mask, batch_size, head_num, seq_len, scalar);
  } else {
  ...
  // cudaMemcpy(dst, qk_buf_, m*seq_len*head_num*sizeof(T),
  // cudaMemcpyDeviceToDevice); return ;

  check_cuda_error(cublasGemmStridedBatchedEx(
      cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, size_per_head, seq_len, seq_len,
      &alpha, v_buf_, AType_, size_per_head, seq_len * size_per_head, qk_buf_,
      BType_, seq_len, seq_len * seq_len, &beta, transpose_dst_, CType_,
      size_per_head, seq_len * size_per_head, batch_size * head_num,
      computeType_, static_cast<cublasGemmAlgo_t>(cublasAlgo_[2])));
  ...
  if (0) {
    ...
    transpose<T><<<grid, block, 0, stream>>>(
        transpose_dst_, dst, batch_size, seq_len, head_num, size_per_head / 2);
  } else {
...
template <typename T>
void
OpenMultiHeadAttention(
    cublasHandle_t& cublas, const int B, const int L, const int numHeads,
    const int headSize, const float rsqrtHeadSize,
    TransformerEncodeNode<T>& nodes, TransformerEncodeTemp<T>& temps,
    cudaStream_t stream)
{
  ...
  check_cuda_error(cublasGemmEx(
      cublas, CUBLAS_OP_N, CUBLAS_OP_N, n, m, k, &alpha,
      nodes.self_attention.query_weight.kernel, AType, n, temps.att_in_buf0,
      BType, k, &beta, temps.q_buf0, CType, n, computeType,
      static_cast<cublasGemmAlgo_t>(cublasAlgo_[0])));

  check_cuda_error(cublasGemmEx(
      cublas, CUBLAS_OP_N, CUBLAS_OP_N, n, m, k, &alpha,
      nodes.self_attention.key_weight.kernel, AType, n, temps.att_in_buf1,
      BType, k, &beta, temps.k_buf0, CType, n, computeType,
      static_cast<cublasGemmAlgo_t>(cublasAlgo_[0])));

  check_cuda_error(cublasGemmEx(
      cublas, CUBLAS_OP_N, CUBLAS_OP_N, n, m, k, &alpha,
      nodes.self_attention.value_weight.kernel, AType, n, temps.att_in_buf1,
      BType, k, &beta, temps.v_buf0, CType, n, computeType,
      static_cast<cublasGemmAlgo_t>(cublasAlgo_[0])));

  multiHeadAttr_nofuse_kernelLauncher<T>(
      stream, cublas, temps.q_buf0, temps.k_buf0, temps.v_buf0, temps.q_buf1,
      temps.k_buf1, temps.v_buf1, temps.qk_buf0, temps.transpose_dst,
      nodes.att_mask, temps.att_out_buf, B, L, numHeads, headSize,
      rsqrtHeadSize);
}
...
transEncode(
    cublasHandle_t& cublas, const int B, const int L, const int numHeads,
    const int headSize, const float rsqrtHeadSize, bool is_last_layer,
    TransformerEncodeNode<T>& nodes, TransformerEncodeTemp<T>& temps,
    cudaStream_t stream)
{
  cudaDataType_t computeType = CUDA_R_32F;
  ...
  // print_abs_mean((T *) nodes.encode_input, num, stream);
  ...
  check_cuda_error(cublasGemmEx(
      cublas, CUBLAS_OP_N, CUBLAS_OP_N, n, m, k, &alpha,
      nodes.self_conv1d0.kernel, AType, n, temps.im2col_buf0, BType, k, &beta,
      temps.conv1d_buf0, CType, n, computeType,
      static_cast<cublasGemmAlgo_t>(cublasAlgo_[0])));

  k = numHeads * headSize;
  n = k;
  add_bias_kernelLauncher<T>(
      temps.conv1d_buf0, nodes.self_conv1d0.bias, m, n, true, stream);

  layernorm_kernelLauncher<T>(
      temps.layernorm_buf0, temps.conv1d_buf0, nodes.self_layernorm0.gamma,
      nodes.self_layernorm0.beta, m, n, stream);

  im2col1d_gpu<T>(
      temps.layernorm_buf0, temps.im2col_buf1, k, m, L, 3, 1, stream);

  k = numHeads * headSize * 3;
  check_cuda_error(cublasGemmEx(
      cublas, CUBLAS_OP_N, CUBLAS_OP_N, n, m, k, &alpha,
      nodes.self_conv1d1.kernel, AType, n, temps.im2col_buf1, BType, k, &beta,
      temps.conv1d_buf1, CType, n, computeType,
      static_cast<cublasGemmAlgo_t>(cublasAlgo_[1])));

  k = numHeads * headSize;
  n = k;
  add_bias_kernelLauncher<T>(
      temps.conv1d_buf1, nodes.self_conv1d1.bias, m, n, true, stream);

  layernorm_kernelLauncher(
      temps.layernorm_buf1, temps.conv1d_buf1, nodes.self_layernorm1.gamma,
      nodes.self_layernorm1.beta, m, n, stream);

  add_op<T>(
      temps.add_buf0, temps.layernorm_buf1, nodes.encode_input,
      buf_size / wordSize, stream);

  layernorm_kernelLauncher<T>(
      temps.layernorm_buf2, temps.add_buf0, nodes.self_layernorm2.gamma,
      nodes.self_layernorm2.beta, m, n, stream);

  check_cuda_error(cudaMemcpy(
      temps.att_in_buf0, temps.layernorm_buf2, buf_size,
      cudaMemcpyDeviceToDevice));
  check_cuda_error(cudaMemcpy(
      temps.att_in_buf1, temps.layernorm_buf2, buf_size,
      cudaMemcpyDeviceToDevice));

  OpenMultiHeadAttention(
      cublas, B, L, numHeads, headSize, rsqrtHeadSize, nodes, temps, stream);

  check_cuda_error(cublasGemmEx(
      cublas, CUBLAS_OP_N, CUBLAS_OP_N, n, m, k, &alpha,
      nodes.self_attention.attention_output_weight.kernel, AType, n,
      temps.att_out_buf, BType, k, &beta, temps.att_matmul_buf, CType, n,
      computeType, static_cast<cublasGemmAlgo_t>(cublasAlgo_[2])));

  add_op(
      temps.add_buf1, temps.att_matmul_buf, temps.add_buf0, buf_size / wordSize,
      stream);
  layernorm_kernelLauncher(
      temps.ffn_layernorm_buf, temps.add_buf1, nodes.ffn_layernorm.gamma,
      nodes.ffn_layernorm.beta, m, n, stream);

  n *= 2;
  check_cuda_error(cublasGemmEx(
      cublas, CUBLAS_OP_N, CUBLAS_OP_N, n, m, k, &alpha,
      nodes.ffn.intermediate_weight.kernel, AType, n, temps.ffn_layernorm_buf,
      BType, k, &beta, temps.ffn_inter_buf, CType, n, computeType,
      static_cast<cublasGemmAlgo_t>(cublasAlgo_[3])));

  add_bias_kernelLauncher<T>(
      temps.ffn_inter_buf, nodes.ffn.intermediate_weight.bias, m, n, true,
      stream);
  ...
  check_cuda_error(cublasGemmEx(
      cublas, CUBLAS_OP_N, CUBLAS_OP_N, n, m, k, &alpha,
      nodes.ffn.output_weight.kernel, AType, n, temps.ffn_inter_buf, BType, k,
      &beta, temps.ffn_out_buf, CType, n, computeType,
      static_cast<cublasGemmAlgo_t>(cublasAlgo_[4])));
  add_bias_kernelLauncher<T>(
      temps.ffn_out_buf, nodes.ffn.output_weight.bias, m, n, false, stream);

  if (is_last_layer) {
    add_mask_op(
        temps.add_buf2, temps.add_buf1, temps.ffn_out_buf, nodes.ffn_mask,
        buf_size / wordSize, stream);
    layernorm_kernelLauncher(
        temps.layernorm_final_buf, temps.add_buf2, nodes.layernorm_final.gamma,
        nodes.layernorm_final.beta, m, n, stream);
    check_cuda_error(cudaMemcpy(
        nodes.encode_out, temps.layernorm_final_buf, buf_size,
        cudaMemcpyDeviceToDevice));
  } else {
    add_mask_op(
        nodes.encode_out, temps.add_buf1, temps.ffn_out_buf, nodes.ffn_mask,
        buf_size / wordSize, stream);
  }

  // printf("encode_output:\n");
  // print_abs_mean((T*)nodes.encode_out, num, stream);
  return status;
}

template <typename T>
TransformerEncodeNode<T>::TransformerEncodeNode(
    const void* const* inputs, void* const* outputs)
{
  ...
  self_attention.attention_output_weight.kernel =
      static_cast<const T*>(inputs[16]);
...
template <typename T>
TransformerEncodeTemp<T>::TransformerEncodeTemp(
    const int B, const int L, const int numHeads, const int headSize,
    void* workspace)
{
...
static const char* TRANSFORMER_ENCODE_PLUGIN_NAME{
    "TransformerEncodePluginDynamic"};
}  // namespace

TransformerEncodePluginDynamic::TransformerEncodePluginDynamic(
    const std::string name, const DataType type, const int hiddenSize,
    const int numHeads, const bool isLastLayer)
    : mLayerName(name), mHiddenSize(hiddenSize), mNumHeads(numHeads),
      mType(type), mIsLastLayer(isLastLayer)
{
...
TransformerEncodePluginDynamic::clone() const noexcept
{
  auto ret = new TransformerEncodePluginDynamic(
      mLayerName, mType, mHiddenSize, mNumHeads, mIsLastLayer);
  ret->initialize();
  return ret;
}

DimsExprs
TransformerEncodePluginDynamic::getOutputDimensions(
...
bool
TransformerEncodePluginDynamic::supportsFormatCombination(
...
void
TransformerEncodePluginDynamic::configurePlugin(
...
size_t
TransformerEncodePluginDynamic::getWorkspaceSize(
    ...
    const PluginTensorDesc* outputs, int nbOutputs) const noexcept
{
  ...
  // 一开始给中间变量分配好内存空间
  const size_t ws =
      31 * B * L * mHiddenSize * wordSize + B * L * L * mNumHeads * wordSize;
  std::cout << "encode workspace:" << ws << " B:" << B << " L:" << L
            << " wordSize:" << wordSize << " hidden_units:" << mHiddenSize
            << std::endl;
  return ws;
}
...
DataType
TransformerEncodePluginDynamic::getOutputDataType(
...
// IPluginV2 Methods
const char*
TransformerEncodePluginDynamic::getPluginType() const noexcept
{
  return TRANSFORMER_ENCODE_PLUGIN_NAME;
}

const char*
TransformerEncodePluginDynamic::getPluginVersion() const noexcept
{
  return TRANSFORMER_ENCODE_PLUGIN_VERSION;
}

int
TransformerEncodePluginDynamic::getNbOutputs() const noexcept
{
  return 1;
}

int
TransformerEncodePluginDynamic::initialize() noexcept
{
  cublasCreate(&cublas);
  return 0;
}
...
void
TransformerEncodePluginDynamic::terminate() noexcept
{
  PLUGIN_CHECK(cublasDestroy(cublas));
}

size_t
TransformerEncodePluginDynamic::getSerializationSize() const noexcept
{
  return sizeof(DataType) + sizeof(mNumHeads) + sizeof(mHeadSize) +
         sizeof(mRsqrtHeadSize) + sizeof(mHiddenSize) + sizeof(mIsLastLayer);
}

void
TransformerEncodePluginDynamic::serialize(void* buffer) const noexcept
{
...
void
TransformerEncodePluginDynamic::destroy() noexcept
{
  delete this;
}

void
TransformerEncodePluginDynamic::setPluginNamespace(
...
const char*
TransformerEncodePluginDynamic::getPluginNamespace() const noexcept
{
  return mNamespace.c_str();
}

int
TransformerEncodePluginDynamic::enqueue(
    ...
    cudaStream_t stream) noexcept
{
  ...
  if (mType == DataType::kFLOAT) {
    ...
    status = transEncode<float>(
        cublas, B, L, mNumHeads, mHeadSize, mRsqrtHeadSize, mIsLastLayer, nodes,
        temps, stream);
  } else {
...
std::vector<PluginField>
    TransformerEncodePluginDynamicCreator::mPluginAttributes;
...
TransformerEncodePluginDynamicCreator::TransformerEncodePluginDynamicCreator()
{
  ...
  mPluginAttributes.emplace_back(
      PluginField("isLastLayer", nullptr, PluginFieldType::kINT32, 1));
  mFC.nbFields = mPluginAttributes.size();
  mFC.fields = mPluginAttributes.data();
}

const char*
TransformerEncodePluginDynamicCreator::getPluginName() const noexcept
{
  return TRANSFORMER_ENCODE_PLUGIN_NAME;
}

const char*
TransformerEncodePluginDynamicCreator::getPluginVersion() const noexcept
{
  return TRANSFORMER_ENCODE_PLUGIN_VERSION;
}

const PluginFieldCollection*
TransformerEncodePluginDynamicCreator::getFieldNames() noexcept
{
  return &mFC;
}

IPluginV2*
TransformerEncodePluginDynamicCreator::createPlugin(
    const char* name, const PluginFieldCollection* fc) noexcept
{
  ...
  if (hiddenSize <= 0) {
    std::cout << "TransformerEncode: Invalid hiddenSize " << hiddenSize
              << std::endl;
  }
  if (numHeads <= 0) {
    std::cout << "TransformerEncode: Invalid numHeads " << numHeads
              << std::endl;
  }
  if (isLastLayer < 0) {
    std::cout << "TransformerEncode: Invalid isLastLayer " << isLastLayer
              << std::endl;
  }

  // std::cout << "typeId:" << typeId << "hidden_size:" << hiddenSize <<
  // "num_heads:" << numHeads << "isLastLayer:" << isLastLayer << std::endl;
  DataType type = static_cast<DataType>(typeId);
  TransformerEncodePluginDynamic* p = new TransformerEncodePluginDynamic(
      name, type, hiddenSize, numHeads, isLastLayer);
  return p;
}

IPluginV2*
TransformerEncodePluginDynamicCreator::deserializePlugin(
...
void
TransformerEncodePluginDynamicCreator::setPluginNamespace(
    const char* libNamespace) noexcept
{
  mNamespace = libNamespace;
}
...
const char*
TransformerEncodePluginDynamicCreator::getPluginNamespace() const noexcept
{
  return mNamespace.c_str();
}
